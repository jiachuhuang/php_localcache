...
/* MurmurHash2
 */
static inline unsigned int c_cache_hash_func(const char *data, unsigned int len) {
...
static inline unsigned int c_storage_crc32(char *data, unsigned int size) /* {{{ */ {
    if (size < C_STORAGE_CRC_THRESHOLD) {
        return crc32(data, size);
    } else {
        int i = 0;
        char crc_contents[C_STORAGE_CRC_THRESHOLD];
        int head = C_STORAGE_CRC_THRESHOLD >> 2;
        int tail = C_STORAGE_CRC_THRESHOLD >> 4;
        int body = C_STORAGE_CRC_THRESHOLD - head - tail;
        char *p = data + head;
        ...
        return crc32(crc_contents, C_STORAGE_CRC_THRESHOLD);
    }
}
/* }}} */
...
int c_storage_find(char *key, unsigned int len, void **data, unsigned int *size,/* unsigned int *flag,*/ unsigned long tv) {
    ...
    k = (c_kv_key*)(p + k_offset + k_index * kv_len);

    if(k->val.len) {
        if(k->h != hash || k->len != len || memcmp(key, k->key, len) || k->ttl < tv) {
            goto miss;
        }
        ...
        v = USE_MALLOC(k->val.len);
        memcpy(v, (shared_segments)[k->val.segment_index].p + k->val.offset, k->val.len);
...
int c_storage_update(char *key, unsigned int len, void *data, unsigned int size, unsigned int ttl,/* unsigned int *flag,*/ unsigned int add, unsigned long tv) {
    ...
    if(len > C_STORAGE_MAX_KEY_LEN || size > (C_STORAGE_SEGMENT_SIZE(shared_header) - sizeof(c_shared_segment_header)) ) {
        return C_CACHE_FAIL;
    }
    ...
    if(k->val.len) {
        // maybe set
        if( k->len != len || memcmp(key, k->key, len) || k->ttl < tv) {
            goto insert;
        }
...
insert: 
        ...
        k->val.offset = offset;
        k->val.len = size;
        k->val.segment_index = segment_index;
        k->val.atime = tv;

        k->crc = c_storage_crc32((char*)data, size);
